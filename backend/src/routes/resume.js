...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Apply the selected rewrites to the raw resume text.
    const newRaw = applyRewritesToText(baseVersion.rawText, selected);

    // Safety net:
    // Pre-build a structured copy from the base version with
    // the chosen bullets swapped in. This prevents V2 from
    // becoming empty if Gemini's re-parse fails.
    const patchedFromBase = patchBulletsInSections(
      baseVersion.parsedSections,
      selected,
    );

    // Re-parse the updated resume.
    const reparsed = await parseStructured(newRaw);

    // If the AI parser returns empty sections, use the
    // patched version from the original structured data.
