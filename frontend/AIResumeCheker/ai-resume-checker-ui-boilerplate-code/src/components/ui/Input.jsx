...
export const Input = forwardRef(
  ({ className, type = "text", ...props }, ref) => (
    <input
      ...
      type={type}
      className={cn(
        "h-10 w-full rounded-full border border-[var(--border)] " +
          "bg-[var(--surface)] px-4 text-sm text-[var(--ink)] " +
          "placeholder:text-[var(--ink-muted)] outline-none transition-colors " +
...
export const SearchInput = forwardRef(
  ({ className, leftIcon, rightSlot, ...props }, ref) => (
    <div
      className={cn(
        "group flex items-center gap-3 h-11 rounded-full " +
          "bg-[var(--surface)] border border-[var(--border)] " +
          "pl-5 pr-1.5 shadow-card transition-all " +
          "hover:shadow-hover " +
